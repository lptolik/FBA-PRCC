...
alFlOutl<- paste0('|p{0.01\\textwidth}',#ID '|l',#
           '|p{0.35\\textwidth}',                #Strain 
           '|p{0.15\\textwidth}',                #MetID 
           '|r',                #N 
           '|r',                #Nr
           '|r',                #Nucard 
...
printTable(subTbl10[order(pval,-1*original)], digits = 5,
           main = 'Substrates for the $\\Delta hisD$ model with blocked AlaHis assimilation',
           landscape = TRUE,
           #align = 'llllll',
           align = alEdgesOne,
...
printTable(prdTbl10[order(pval,-1*original)], digits = 5,
           main = 'Products for the $\\Delta hisD$ model with blocked AlaHis assimilation',
           landscape = TRUE,
           #align = 'llllll',
           align = alEdgesOne,
...
printTable(attrTbl10[order(pval,original)], digits = 5,
           main = 'Attractants for the $\\Delta hisD$ model with blocked AlaHis assimilation',
           landscape = TRUE,
           #align = 'llllll',
           align = alEdgesOne,
...
printTable(repTbl10[order(pval,original)], digits = 5,
           main = 'Repellents for the $\\Delta hisD$ model with blocked AlaHis assimilation',
           landscape = TRUE,
           #align = 'llllll',#
           align = alEdgesOne,
...
prnTbl<-df[(card>1),c(1:3,6,32,33,35,37,8)]
prnTbl10<-prnTbl[(pval<=0.1)]
pander(table(sign(prnTbl10$original),sub('.+_(.+)$','\\1',prnTbl10$varname)),
...
# Sensome Outlook
Supplementary Table \ref{tab:FlOutline} summarize the properties of model 
parameters for each of four models considered so far. 
```{r prepare.flux.stat}
fl.stat<-dfAll[,.(N=length(NActive64),
                  Nr=length(which(type=='u')),
                  NPcard=length(which(card==1)),NRcard=length(unique(MetID[which(card==1)])),
                  NPrange=length(which(range<=1e-5)),NRrange=length(unique(MetID[which(range<=1e-5)])),
                  NPactive=length(which(NActive64>0)),
                  NRactive=length(unique(MetID[which(NActive64>0)])),
                  NPsig=length(which(pval<=0.1)),NRsig=length(unique(MetID[which(pval<=0.1)]))),
               by=.(Strain,MicrobeID)]
```

```{r fluxes.stat,results='asis',cache=FALSE}
printTable(fl.stat, digits = 5,
           main = 'Flux statistics overview',
           landscape = TRUE,align = alFlOutl,
           label = 'tab:FlOutline')
```

Columns of the Supplementary Table \ref{tab:FlOutline} contains total number
of parameters in the model (N), number of parameters (NPcard) variation of which do not
change the flux of the corresponding reaction (NRcard, calculated as the number of parameters with
unique flux value), 
number of parameters (NPrange), variation of which cause only small variation of the flux
through corresponding reaction (NRrange), number of parameters (NPactive) that have nonempty set of
active boundary values among tested Sobol points and number of correspondent reactions (NRactive),
number of parameters with statistically significant sensitivity coefficient (NPsig) and
number of correspondent reactions (NRsig).
...
df1<-dfw[match(ints$`wt:hisD:trpB`,dfw$varname),c("MetID","varname","Name","ExEquation")]
df1$Group<-'wt:hisD:trpB'
df2<-dfw[match(ints$`wt:hisD2:trpB`,dfw$varname),c("MetID","varname","Name","ExEquation")]
df2$Group<-'wt:hisD2:trpB'
df3<-dfw[match(ints$trpB,dfw$varname),c("MetID","varname","Name","ExEquation")]
df3$Group<-'trpB'
df4<-dfw[match(ints$hisD2,dfw$varname),c("MetID","varname","Name","ExEquation")]
df4$Group<-'hisD2'
