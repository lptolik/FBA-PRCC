...
currency metabolites lists from the [GitHub repository](`r cmurl`)in the separate table:
...
To take into account reversibility of some reaction, each reversible reaction is
...
ggnet2(g,mode=l,color='BowTie', color.palette = "Dark2")
```
...
`r cmp$csize[midx]` nodes, covering `r 100*cmp$csize[midx]/vcount(g)`% of the 
...
All nodes in outside of the main connected componet are groped into the Elf Green 
...
reactions and analyse the structure of the clean metabolic graph.
...
First we need to find disconnected componens in the graph and their composition:
...
ggnet2(gc,mode=l,color='BowTie', color.palette = "Dark2")
```
...
It could be seen that there is much more disconnected componens in the clean 
graph (`r ccmp$no` vs. `r cmp$no`), which contains `r nrow(rndf)` reactions and 
`r nrow(mndf)` metabolites. In the tables above (Supplementary Table S\ref{tab:clnCmpRe} 
and Supplementary Table S\ref{tab:clnCmpMet}) columns `glbComp` and `curComp`
contain number of the weak component in the whole and clean graphs correspondently.
...
calculated by the BioNAR package.
...
### Read data
```{r read.rds.wt,eval=TRUE,cache=FALSE}
...
tdccdf$X<-c(8,16,32,64)*8192
qplot(X,pearson,data=tdccdf)+geom_line()+theme_bw()
```

```{r tdcc.table,results='asis'}
printTable(tdccdf,'TDCC values at different number of Sobol points',
...
```{r make.data.table.wt,eval=TRUE,cache=FALSE}
...
```
```{r read.active.df32.wt,echo=FALSE,cache=FALSE,eval=TRUE}
...
```{r read.active.df64.wt,echo=FALSE,cache=FALSE,eval=TRUE}
dtWT64<-dt
...
```
{r plot.actbound.distr.wt}
actCount=rowSums(dtWT64[,grep('^act',names(dtWT64)),with=FALSE])
qplot(actCount,bins=(1+max(actCount)),
      main = 'Active bounds number for wild type')+theme_bw()
qplot(actCount,bins=(1+max(actCount)),log = 'y',
      main = 'Active bounds number for wild type')+theme_bw()
```

```
{r save.flux_act.wt}
fwrite(df,file = paste0(file.path(mdir,d),'_nz_prcc.csv.gz'),row.names = FALSE,col.names = TRUE)
```
...
```
```{r hist.plot.wt,fig.width=8,fig.height=8}
qplot(df$pval)+
    xlab('p-value')+
...
```
```
{r plot.wt.his.solvsflux,fig.format='png',fig.dpi=600}
qplot(Solution,influx_HisL,data = pltDFwt,color = activityInflux,
      main =bquote("Histidine influx in wildtype"))
qplot(Solution,influx_HisL,data = pltDFwt,facets = activityInflux ~ .,
      color = activityInflux,
      main =bquote("Histidine influx in wildtype"))
```
```{r plot.wt.his.bndvssol,fig.format='png',fig.dpi=600}
qplot(inBound_HisL,Solution,data = pltDFwt,color=activityInflux,log='x',
      main =bquote("Histidine influx boundary in wildtype"))
qplot(inBound_HisL,Solution,data = pltDFwt,color=activityInflux,log='x',
      facets = activityInflux ~ .,
      main =bquote("Histidine influx boundary in wildtype"))
```

```{r plot.wt.his.bndvsinflux,fig.format='png',fig.dpi=600}
...
```
```{r plot.wt.trpB.solvsflux,fig.format='png',fig.dpi=600}
qplot(Solution,influx_TrpL,data = pltDFtrp,color = activityInflux,
      main =bquote("Tryptophan influx in wildtype"))
qplot(Solution,influx_TrpL,data = pltDFtrp,facets = activityInflux ~ .,
      color = activityInflux,
      main =bquote("Tryptophan influx in wildtype"))
```
```{r plot.wt.trpB.bndvssol,fig.format='png',fig.dpi=600}
qplot(inBound_TrpL,Solution,data = pltDFtrp,color=activityInflux,log='x',
      main =bquote("Tryptophan influx boundary in wildtype"))
qplot(inBound_TrpL,Solution,data = pltDFtrp,color=activityInflux,log='x',
      facets = activityInflux ~ .,
      main =bquote("Tryptophan influx boundary in wildtype"))
```

```{r plot.wt.trpB.bndvsinflu,fig.format='png',fig.dpi=600}
...
pander(table(sign(prnTbl10$original),sub('.+_(.+)$','\\1',prnTbl10$varname)),
       caption = 'Sign/bound frequency table for wild-type model')
```
...
### Read data
```{r read.rds.hisD,eval=TRUE,cache=FALSE}
...
```{r make.data.table.hisD,eval=TRUE,cache=FALSE}
df<-as.data.table(frdf)
df$HMDF["['NaN']"==df$HMDF]<-""
df$ChEBI["['NaN']"==df$ChEBI]<-""
df$InChI["['NaN']"==df$InChI]<-""
...
df$NActive16[idx]<-apply(as.data.frame(dt)[1:131072,-c(1,ncol(dt))],2,sum)
```
```{r read.active.df32.hisD,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,mean)
df$NActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,sum)
```

```{r read.active.df64.hisD,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,mean)
df$NActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,sum)
```
...
```
{r plot.actbound.distr.hisD}
actCount=rowSums(dtHisD64[,grep('^act',names(dtHisD64)),with=FALSE])
qplot(actCount,bins=(1+max(actCount)),
      main = 'Active bounds number for hisD mutant')+theme_bw()
qplot(actCount,bins=(1+max(actCount)),log = 'y',
      main = 'Active bounds number for hisD mutant')+theme_bw()
```

```
{r save.flux_act.hisD}
fwrite(df,file = paste0(file.path(mdir,d),'_nz_prcc.csv.gz'),row.names = FALSE,col.names = TRUE)
```
...
#dfw<-merge(dfw,df,by='varname',all.x=TRUE)
dfw<-merge(dfw,df,by=mvar,all.x=TRUE,suffixes = c('.wt',''))
#names(dfw)<-sub('.x','.wt',names(dfw))
#names(dfw)<-sub('.y','',names(dfw))
```
```{r hist.plot.hisD,fig.width=8,fig.height=8}
qplot(df$pval)+
    xlab('p-value')+
...
```
```{r plot.hisD.solvsflux,fig.format='png',fig.dpi=600}
qplot(Solution,influx_HisL,data = pltDFhis,color = activityInflux,
      main =bquote("Histidine influx in "~Delta~"hisD mutant"))
qplot(Solution,influx_HisL,data = pltDFhis,facets = activityInflux ~ .,
      color = activityInflux,
      main =bquote("Histidine influx in "~Delta~"hisD mutant"))
```
```{r plot.hisD.bndvssol,fig.format='png',fig.dpi=600}
qplot(inBound_HisL,Solution,data = pltDFhis,color=activityInflux,log='x',
      main =bquote("Histidine influx boundary in "~Delta~"hisD mutant"))
qplot(inBound_HisL,Solution,data = pltDFhis,color=activityInflux,log='x',
      facets = activityInflux ~ .,
      main =bquote("Histidine influx boundary in "~Delta~"hisD mutant"))
```

```{r plot.hisD.bndvsinflux,fig.format='png',fig.dpi=600}
...
### Read data
```{r read.rds.hisD.noalahis,eval=TRUE,cache=FALSE}
...
```{r make.data.table.hisD.noalahis,eval=TRUE,cache=FALSE}
df<-as.data.table(frdf)
df$HMDF["['NaN']"==df$HMDF]<-""
df$ChEBI["['NaN']"==df$ChEBI]<-""
df$InChI["['NaN']"==df$InChI]<-""
...
df$NActive16[idx]<-apply(as.data.frame(dt)[1:131072,-c(1,ncol(dt))],2,sum)
```
```{r read.active.df32.hisD.noalahis,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,mean)
df$NActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,sum)
```

```{r read.active.df64.hisD.noalahis,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,mean)
df$NActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,sum)
```
...
```
{r plot.actbound.distr.hisD.noalahis}
actCount=rowSums(dtHisD64[,grep('^act',names(dtHisD64)),with=FALSE])
qplot(actCount,bins=(1+max(actCount)),
      main = 'Active bounds number for hisD mutant')+theme_bw()
qplot(actCount,bins=(1+max(actCount)),log = 'y',
...
#names(dfw)<-sub('.x','.hisD',names(dfw))
#names(dfw)<-sub('.y','',names(dfw))
#names(dfw)<-sub('.y','.hisD2',names(dfw))
#write.csv(dfw,file='ecoliK12.widePRCC_no_biomass64.csv')
```


```{r hist.plot.hisD.noalahis,fig.width=8,fig.height=8}
qplot(df$pval)+
    xlab('p-value')+
...
```
```
{r plot.hisD.noalahis.solvsflux,fig.format='png',fig.dpi=600}
qplot(Solution,influx_HisL,data = pltDFhis,color = activityInflux,
      main =bquote("Histidine influx in "~Delta~"hisD mutant"))
qplot(Solution,influx_HisL,data = pltDFhis,facets = activityInflux ~ .,
      color = activityInflux,
      main =bquote("Histidine influx in "~Delta~"hisD mutant"))
```
```{r plot.hisD.noalahis.bndvssol,fig.format='png',fig.dpi=600}
qplot(inBound_HisL,Solution,data = pltDFhis,color=activityInflux,log='x',
      main =bquote("Histidine influx boundary in "~Delta~"hisD mutant"))
qplot(inBound_HisL,Solution,data = pltDFhis,color=activityInflux,log='x',
      facets = activityInflux ~ .,
...
```{r plot.hisD.noalahis.bndvsinflux,fig.format='png',fig.dpi=600}
qplot(inBound_HisL,influx_HisL,data = pltDFhis,color = activityInflux,log='x',
      main =bquote("Histidine influx in "~Delta~"hisD mutant"))
qplot(inBound_HisL,influx_HisL,data = pltDFhis,color = activityInflux,log='x',
      facets = activityInflux ~ .,
...
## Read data
```{r read.rds.trpB,eval=TRUE,cache=FALSE}
...
```{r make.data.table.trpB,eval=TRUE,cache=FALSE}
df<-as.data.table(frdf)
df$HMDF["['NaN']"==df$HMDF]<-""
df$ChEBI["['NaN']"==df$ChEBI]<-""
df$InChI["['NaN']"==df$InChI]<-""
...
df$NActive16[idx]<-apply(as.data.frame(dt)[1:131072,-c(1,ncol(dt))],2,sum)
```
```{r read.active.df32.trpB,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,mean)
df$NActive32[idx]<-apply(as.data.frame(dt)[1:262144,-c(1,ncol(dt))],2,sum)
```

```{r read.active.df64.trpB,echo=FALSE,cache=FALSE,eval=TRUE}
df$PercActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,mean)
df$NActive64[idx]<-apply(as.data.frame(dt)[,-c(1,ncol(dt))],2,sum)
```

```
{r plot.actbound.distr.trpB}
actCount=rowSums(dtTrpB64[,grep('^act',names(dtTrpB64)),with=FALSE])
qplot(actCount,bins=(1+max(actCount)),
      main = 'Active bounds number for trpB mutant')+theme_bw()
qplot(actCount,bins=(1+max(actCount)),log = 'y',
      main = 'Active bounds number for trpB mutant')+theme_bw()
```

```
{r save.flux_act.trpB}
fwrite(df,file = paste0(file.path(mdir,d),'_nz_prcc.csv.gz'),row.names = FALSE,col.names = TRUE)
```
...
#names(dfw)<-sub('.x','.hisD2',names(dfw))
#names(dfw)<-sub('.y','.trpB',names(dfw))
#write.csv(dfw,file='ecoliK12.widePRCC_no_biomass64.csv')
```
```{r hist.plot.trpB,fig.width=8,fig.height=8}
qplot(df$pval)+
    xlab('p-value')+
...
```
```
{r plot.trpB.solvsflux,fig.format='png',fig.dpi=600}
qplot(Solution,influx_TrpL,data = pltDFtrp,color = activityInflux,
      main =bquote("Tryptophan influx in "~Delta~"trpB mutant"))
qplot(Solution,influx_TrpL,data = pltDFtrp,facets = activityInflux ~ .,
      color = activityInflux,
      main =bquote("Tryptophan influx in "~Delta~"trpB mutant"))
```
```{r plot.trpB.bndvssol,fig.format='png',fig.dpi=600}
qplot(inBound_TrpL,Solution,data = pltDFtrp,color=activityInflux,log='x',
      main =bquote("Tryptophan influx boundary in "~Delta~"trpB mutant"))
qplot(inBound_TrpL,Solution,data = pltDFtrp,color=activityInflux,log='x',
      facets = activityInflux ~ .,
      main =bquote("Tryptophan influx boundary in "~Delta~"trpB mutant"))
```

```{r plot.trpB.bndvsinflux,fig.format='png',fig.dpi=600}
qplot(inBound_TrpL,influx_TrpL,data = pltDFtrp,color = activityInflux,log='x',
      main =bquote("Tryptophan influx in "~Delta~"trpB mutant"))
qplot(inBound_TrpL,influx_TrpL,data = pltDFtrp,color = activityInflux,log='x',
      facets = activityInflux ~ .,
...
# Inactive constraint intersections
```
{r inact.upset}
inactUpset<-list(wt=dfw$varname[(dfw$PercActive32.wt==0)&(dfw$card.wt>1)],
                 hisD=dfw$varname[(dfw$PercActive32.hisD==0)&(dfw$card.hisD>1)],
                 trpB=dfw$varname[(dfw$PercActive32.trpB==0)&(dfw$card.trpB>1)])
upset(fromList(inactUpset))
```

```
{r inact.venn,fig.height=6,fig.width=6}
gplots::venn(inactUpset)
```

# Active constraint sensomes intersections
```
{r act.upset}
actUpset<-list(wt=dfw$varname[(dfw$pval.wt<0.10)&(dfw$PercActive32.wt>0)&(dfw$card.wt>1)],
                 hisD=dfw$varname[(dfw$pval.hisD<0.10)&(dfw$PercActive32.hisD>0)&(dfw$card.hisD>1)],
                 trpB=dfw$varname[(dfw$pval.trpB<0.10)&(dfw$PercActive32.trpB>0)&(dfw$card.trpB>1)])
upset(fromList(actUpset))
```

```
{r act.venn,fig.height=6,fig.width=6}
gplots::venn(actUpset)
```
...
## Bidirectional metabolites at P-val 0.1
```{r bidir.met.p10}
...
metCar<-fmdf10[,.(N=length(unique(Strain)),
                  Nc_16=length(unique(Strain[which(card==1)])),
                  #Nc_64=length(unique(Strain[which(card_64==1)])),
                  Nr_16=length(unique(Strain[which(range<=1e-5)])),
                  #Nr_64=length(unique(Strain[which(range_64<=1e-5)])),
...
printTable(metCar[order(metCar$N,decreasing = TRUE),],
           main = 'Frequency of bidirectional metabolites pval=0.1',
           landscape = FALSE,align = alMetCarF)

```

## Bidirectional metabolites at P-val 0.05
```{r bidir.met.p5}
mdf5<-df5[,.(N=length(unique(varname))),by=.(Strain,MetID,Name)][N>1]
fmdf5<-merge(mdf5,df5,by=c('Strain','MetID','Name'))
dlgpval5<-fmdf5[,.(dlg=mean(abs(diff(log10(pval)))),
                     sgn=prod(sign(original))),
                by=.(Strain,MetID,Name)]
fmdf5<-merge(fmdf5,dlgpval5,by=c('Strain','MetID','Name'))
metCar<-fmdf5[,.(N=length(unique(Strain)),
                  Nc_16=length(unique(Strain[which(card==1)])),
                  #Nc_64=length(unique(Strain[which(card_64==1)])),
                  Nr_16=length(unique(Strain[which(range<=1e-5)])),
                  #Nr_64=length(unique(Strain[which(range_64<=1e-5)])),
...
```{r bidir.met.car.table.p5,results='asis',cache=FALSE}
printTable(metCar[order(metCar$N,decreasing = TRUE),],
           main = 'Frequency of bidirectional metabolites pval=0.05',
           landscape = FALSE,align = alMetCarF)

```


## Bidirectional metabolites at P-val 0.01
```{r bidir.met.p1}
mdf1<-df1[,.(N=length(unique(varname))),by=.(Strain,MetID,Name)][N>1]
fmdf1<-merge(mdf1,df1,by=c('Strain','MetID','Name'))
dlgpval1<-fmdf1[,.(dlg=mean(abs(diff(log10(pval)))),
                     sgn=prod(sign(original))),
                by=.(Strain,MetID,Name)]
fmdf1<-merge(fmdf1,dlgpval1,by=c('Strain','MetID','Name'))
metCar<-fmdf1[,.(N=length(unique(Strain)),
                  Nc_16=length(unique(Strain[which(card==1)])),
                  #Nc_64=length(unique(Strain[which(card_64==1)])),
                  Nr_16=length(unique(Strain[which(range<=1e-5)])),
                  #Nr_64=length(unique(Strain[which(range_64<=1e-5)])),
...
```{r bidir.met.car.table.p1,results='asis',cache=FALSE}
printTable(metCar[order(metCar$N,decreasing = TRUE),],
           main = 'Frequency of bidirectional metabolites pval=0.01',
           landscape = FALSE,align = alMetCarF)
```

## Interaction type table for all three models
```{r interaction.types}
#prnTbl<-dfw[,c(1:3,6,33,36,62,65,9,11:21)]
...
#prnTbl$type<-sub('.+_(.+)$','\\1',prnTbl$varname)
#prnTbl10<-prnTbl[(pval.wt<=0.1)|(pval.hisD<=0.1)|(pval.trpB<=0.1)]
...
```
{r prepare.ecdf}
ecdfWT<-ecdf(dfSolution[Strain=='wt']$Solution)
ecdfHisD<-ecdf(dfSolution[Strain=='hisD']$Solution)
ecdfTrpB<-ecdf(dfSolution[Strain=='trpB']$Solution)
tt5<-seq(min(dfSolution$Solution),max(dfSolution$Solution),by=5)
vHisD5<-ecdfHisD(tt5)
vTrpB5<-ecdfTrpB(tt5)
vWT5<-ecdfWT(tt5)
```
```
{r plot.ecdf.diff.TrpB}
pltdf<-data.frame(tt=tt5,diffTrpB=(vTrpB5-vWT5),diffHisD=(vHisD5-vWT5))
ggplot(pltdf,aes(x=tt5,y=diffTrpB))+geom_line()+
    xlab('Optimal solution value')+
    ylab(bquote("Difference between "~Delta~"trpB and wild type ECDF"))+
    theme_bw()
```
...
```
{r KS.test.HisD}
ks.test(dfSolution[Strain=='wt']$Solution,dfSolution[Strain=='hisD']$Solution)
```

```
{r KS.test.TrpB}
ks.test(dfSolution[Strain=='wt']$Solution,dfSolution[Strain=='trpB']$Solution)
```

```
{r KS.test.TrpB.less}
ks.test(dfSolution[Strain=='wt']$Solution,dfSolution[Strain=='trpB']$Solution,alternative = 'less')
```
