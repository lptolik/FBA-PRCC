...
qplot(inBound_HisL,Solution,data = pltDFwt,color=activityInflux,log='x',
      main =bquote("Histidine influx boundary in wildtype"))+theme_bw()
```
...
qplot(inBound_HisL,influx_HisL,data = pltDFwt,color = activityInflux,log='x',
      main =bquote("Histidine influx in wildtype"))+theme_bw()
```
...
Supplementary Figures \ref{fig:wt_trp_bs}, \ref{fig:wt_trp_bs_f}, \ref{fig:wt_trp_bf}, and \ref{fig:wt_trp_bf_f} demonstrate how histidine influx bound value influences
histidine influx itself and biomass production.
...
qplot(inBound_TrpL,Solution,data = pltDFtrp,color=activityInflux,log='x',
      main =bquote("Tryptophan influx boundary in wildtype"))+theme_bw()
```
...
qplot(inBound_TrpL,influx_TrpL,data = pltDFtrp,color = activityInflux,log='x',
      main =bquote("Tryptophan influx in wildtype"))+theme_bw()
```
...
```
```{r hist.plot.hisD,fig.width=8,fig.height=8,fig.cap="\\label{fig:wt_prcc_pval} Distribution of the significance of the exchange reaction boundaries PRCC values for the hisD model."}
qplot(df$pval)+
    xlab('p-value')+
...
```
```{r hist.plot.trpB,fig.width=8,fig.height=8,fig.cap="\\label{fig:wt_prcc_pval} Distribution of the significance of the exchange reaction boundaries PRCC values for the trpB model."}
qplot(df$pval)+
    xlab('p-value')+
